...
function applyDataCompat() {
    ...
    P.usd = STANDARDS.fx.USD;
    P.eur = STANDARDS.fx.EUR;
    P.hc = STANDARDS.hourly_cost;
    ...
    for (const [brandKey, brandData] of Object.entries(BRANDS)) {
        for (const [vKey, vData] of Object.entries(brandData.valves)) {
            // dims key'lerini integer'a çevir
            const intDims = {};
...
null.onclick = () => {
    null.classList.toggle('collapsed');
};
...
function calcItem(it){
 const _vd=VALVE_DATA[it.vref||it.ty],_dd=_vd&&_vd.dims&&_vd.dims[it.dn]?_vd.dims[it.dn]:null;
 const gFM=mn2(P.gFL), I=gFM?tk2(gFM.n):20, N=_dd?_dd.D:gfd(it.dn,it.cl), lens=gbl(it.ty,it.dn,it.cl), inch=DN_INCH[it.dn]||0, apv=gap(inch);
...
function toggleCols() {
  colVis.kumas = document.getElementById('chk_kumas').checked;
  colVis.dolgu = document.getElementById('chk_dolgu').checked;
  colVis.dikis = document.getElementById('chk_dikis').checked;
  colVis.pul = document.getElementById('chk_pul').checked;
 ...
 IT.forEach((it,ii)=>{ const ic=it.calc;
  it.parts.forEach((pt,pi)=>{ const pc=pt.calc||{}, f0=pi===0, np=it.parts.length;
   ...
   tr.innerHTML=(f0?`<td class="lb text-indigo-400 font-bold" rowspan="${np}">${P.prefix?P.prefix+'-':''}${it.ref}</td><td class="lb text-[var(--tx)]" rowspan="${np}">${TN[it.ty]}</td>`:'')+
    ...
    (f0?`<td style="text-align:center; font-weight:700" rowspan="${np}">${it.ty==='pad'?'—':'DN'+it.dn}</td><td style="text-align:center" rowspan="${np}">${it.cl}</td><td rowspan="${np}"><input type="number" value="${it.qt}" min="0" style="width:48px;text-align:center; font-weight:700; color:var(--tx)" onchange="IT[${ii}].qt=parseInt(this.value)||0;recalcAll()"></td><td rowspan="${np}"><input type="number" value="${it.df}" min=".5" max="5" step=".1" style="width:48px;text-align:center" class="text-indigo-400 font-bold bg-[rgba(99,102,241,0.1)] rounded" onchange="IT[${ii}].df=parseFloat(this.value)||1;recalcAll()"></td>`:'')+
...
function rMas(){
 ...
 const svcMontaj=SVC.montajGun*SVC.montajGunMly; const svcTotal=SVC.cad+svcMontaj+SVC.nakliye+(SVC.erisim||0);
...
function rPL(){
 ...
 IT.forEach(it=>{const ic=it.calc, q=plQty[it.ref]||1, aq=it.qt*q, uTL=ic.us||0, tTL=uTL*aq; totTL+=tTL;
  ...
  const tr=document.createElement('tr');
  let h=`<td class="lb font-bold text-indigo-400 text-center">${pref}${it.ref}</td><td class="text-center font-bold">${it.ty==='pad'?'—':'DN'+it.dn}</td><td><input value="${it.mh}" onchange="IT.find(x=>x.ref===${it.ref}).mh=this.value" placeholder="Mahal..."></td><td class="lb text-left">${it.label||(it.ty==='pad'?'Düz Yastık':(TN[it.ty]||it.ty)+' Ceketi - DN'+it.dn+' PN'+it.cl)}</td><td class="o text-center font-bold" ondblclick="openQtyModal(${it.ref})" title="Değiştirmek için çift tıkla">${aq}</td>`;
...
function updDN(){const ty=null/*removed:{eid}*/.value; const s=null/*removed:{eid}*/; s.innerHTML='<option value="">DN</option>'; DN_LIST.forEach(dn=>{s.innerHTML+=`<option value="${dn}">DN${dn}</option>`});}

// ÜRÜN EKLEME ALANI: Dirsek eklendiğinde zorluk otomatik 1.5 ataması
function addItem(){
 const ty=null/*removed:{eid}*/.value, dn=parseInt(null/*removed:{eid}*/.value), cl=parseInt(null/*removed:{eid}*/.value), qt=parseInt(null/*removed:{eid}*/.value);
...
function toggleSidebar(){
 const n=document.getElementById('mainSidebar'),closed=n.style.width==='72px';
...
function go(id,el){
 ...
 document.querySelectorAll('.sidebar-link').forEach(s=>{s.className=s.className.replace(/text-indigo-600|bg-indigo-50|dark:bg-indigo-500\/10|dark:text-indigo-400/g,'').replace(/\s+/g,' ').trim();if(!s.className.includes('text-slate-600'))s.classList.add('text-slate-600','dark:text-slate-400');});
 ...
 if(id==='pl')rPL();if(id==='pd')rProd();if(id==='ms')rMas();if(id==='pm')rPers();if(id==='mf')rMat();if(id==='pr'){rPar();rVRef();}
 lucide.createIcons();
}
...
function saveCols(){Object.keys(colVis).forEach(k=>{const c=document.getElementById('chk_'+k);if(c)colVis[k]=c.checked});toggleCols();closePopup('colVisModal');toast('Sütun ayarları kaydedildi');}

// === BRAND DATA (Marka Hiyerarşisi) ===


// === 3-STEP WIZARD ===
// ================================================================
// GELISMIS WIZARD - 3 ASAMALI MARKA-BAZLI
// ================================================================
    ...
    if (wizStep === 1) {
        ...
        for (const [key, eq] of Object.entries(BRAND_INDEX.equipment_types || {})) {
            h += `<div class="wiz-card" onclick="wizPickCat('${key}')">
                <i data-lucide="${eq.icon}" class="w-6 h-6 mx-auto mb-2"></i>
        ...
        // Düz yastık ve dirsek → direkt 3. aşamaya
        ...
        // Bu ekipman tipini üreten markaları bul
        const availBrands = [];
        for (const [bKey, bData] of Object.entries(BRANDS)) {
            const hasType = Object.values(bData.valves).some(v => v.equipment_type === wizCat);
            if (hasType) {
                const modelCount = Object.values(bData.valves).filter(v => v.equipment_type === wizCat).length;
                availBrands.push({ key: bKey, name: bData.brand, count: modelCount });
            }
        }
        ...
        if (isPad) {
            ...
            <div class="grid grid-cols-2 gap-4 mb-4">
                ...
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Kaç Parçaya Bölünsün?</label><input type="number" id="wPadDiv" class="ps font-mono" value="1" min="1" oninput="wizCalcPadPreview()"></div>
            </div>
            ...
            <div class="grid grid-cols-3 gap-4 mb-4">
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Model</label>
                    <select id="wModel" class="ps font-bold text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-500/5 border-indigo-200 dark:border-indigo-500/20" onchange="wizUpdDN()">${modelOpts}</select></div>
                <div><label class="text-xs font-bold text-cyan-600 dark:text-cyan-500 uppercase block mb-1">DN Ebatı</label>
                    <select id="wDN" class="ps font-mono font-bold text-cyan-600 dark:text-cyan-400 border-cyan-200 dark:border-cyan-500/30 bg-cyan-50 dark:bg-cyan-500/5" onchange="wizFillDims()"><option value="">Seçiniz</option></select></div>
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Basınç Sınıfı</label>
                    <select id="wCls" class="ps"><option value="150">PN16 / Cl150</option><option value="300">PN25 / Cl300</option><option value="400">PN40 / Cl400</option><option value="600">PN63 / Cl600</option></select></div>
            </div>
            <div class="grid grid-cols-4 gap-4 mb-4">
                ...
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Zorluk Katsayısı</label>
                    <input type="number" id="wDF" class="ps font-mono" value="${P.gDF}" step="0.1"></div>
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Sol Bindirme (mm)</label>
                    <input type="number" id="wBX" class="ps font-mono" value="${P.lbx}" step="0.1"></div>
                <div><label class="text-xs font-bold text-slate-500 uppercase block mb-1">Sağ Bindirme (mm)</label>
            ...
            <div class="p-4 bg-cyan-50 dark:bg-cyan-500/10 rounded-xl mb-4 border border-cyan-200 dark:border-cyan-500/20">
                <div class="text-xs font-bold text-cyan-600 dark:text-cyan-400 mb-3 flex items-center gap-2"><i data-lucide="ruler" class="w-4 h-4"></i> Veritabanından Çekilen Ölçüler (değiştirilebilir)</div>
                <div class="grid grid-cols-4 gap-3">
                    <div><label class="text-[10px] text-slate-500 block mb-1">Flanş Çapı D (mm)</label><input type="number" id="wDimD" class="ps font-mono text-cyan-600 dark:text-cyan-400 bg-white dark:bg-slate-800 border-cyan-200 dark:border-cyan-500/20" oninput="wizRecalcCirc()"></div>
                    <div><label class="text-[10px] text-slate-500 block mb-1">2 Flanş Arası L (mm)</label><input type="number" id="wDimL" class="ps font-mono text-cyan-600 dark:text-cyan-400 bg-white dark:bg-slate-800 border-cyan-200 dark:border-cyan-500/20"></div>
                    <div><label class="text-[10px] text-slate-500 block mb-1">Vana Yüksekliği H (mm)</label><input type="number" id="wDimH" class="ps font-mono text-cyan-600 dark:text-cyan-400 bg-white dark:bg-slate-800 border-cyan-200 dark:border-cyan-500/20"></div>
                    <div><label class="text-[10px] text-slate-500 block mb-1">Çevre π×D (mm)</label><input type="number" id="wDimC" class="ps font-mono text-cyan-600 dark:text-cyan-400 bg-white dark:bg-slate-800 border-cyan-200 dark:border-cyan-500/20" readonly></div>
                </div>
            </div>
            <div class="border-t border-slate-200 dark:border-slate-700 pt-4">${wizMaterialHTML()}</div>`;
...
function wizMaterialHTML() {
    ...
    const fabOpts = fabrics.map((m, i) => `<option value="${i}">${m.s} - ${m.n}</option>`).join('') || '<option>Kumaş tanımlı değil</option>';
    const filOpts = fills.map((m, i) => `<option value="${i}">${m.s} - ${m.n}</option>`).join('') || '<option>Dolgu tanımlı değil</option>';
    return `<div class="text-xs font-bold text-slate-500 uppercase mb-3">Malzeme Seçimi <span class="font-normal text-slate-400">(Genel ayarlardan gelir, değiştirilebilir)</span></div>
...
function wizPickBrand(brand) {
    // Seçilen marka + ekipman tipinde model var mı kontrol et
    ...
    if (!models.length) { toast('Bu markaya ait ' + (BRAND_INDEX.equipment_types[wizCat]?.name || '') + ' verisi bulunamadı!', 'er'); return; }
    
    // En az bir modelin dims verisi var mı?
    const hasData = models.some(([k, v]) => Object.keys(v.dims || {}).length > 0);
    if (!hasData) { toast('Bu markaya ait ölçü verisi bulunamadı!', 'er'); return; }
...
function wizUpdDN() {
    ...
    // Zorluk katsayısını modelden çek
...
function wizFillDims() {
    ...
    if (cEl && dims.D) cEl.value = Math.round(Math.PI * dims.D);
    else if (cEl) cEl.value = '';
}
...
function wizRecalcCirc() {
    ...
    const cEl = document.getElementById('wDimC');
    if (cEl) cEl.value = d ? Math.round(Math.PI * d) : '';
}
...
function wizCalcPadPreview() {
    ...
    <div class="grid grid-cols-5 gap-3 text-center">
        ...
        <div><div class="font-mono font-bold text-slate-700 dark:text-slate-200">${pieceW}×${h}</div><div class="text-[10px] text-slate-500">Parça Ölçüsü (mm)</div></div>
...
function wizAdd() {
    ...
    const cl = parseInt(document.getElementById('wCls')?.value) || 16;
    ...
    const parts = pn.map((nm, i) => ({
        nm, ov: {}, bw: P.lbw, bx: bx, by: i === 0 ? by : P.lbyf, calc: {}, matOv: {}
    }));
    ...
    IT.push({
        ref: RN++, ty, vref: ty, dn, cl, qt, df, parts, mh: '', calc: {},
        ovD, ovL, ovH, _brand: wizBrand, _brandName: brandName
    });
...
window.addEventListener('load', async () => {
    ...
    document.getElementById('themeBtn').innerHTML = `<i data-lucide="${isDk ? 'sun' : 'moon'}" class="w-4 h-4"></i>`;
    
    // 4. Checkbox'ları sync et
