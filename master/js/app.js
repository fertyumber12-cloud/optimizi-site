...
// ====== DÖVİZ APİ (Çoklu Fallback) ======
async function fetchKur() {
    const badge = document.getElementById('apiStatusBadge');
    const sources = [
        { url:'https://finans.truncgil.com/v4/today.json', parse:d=>({usd:parseFloat(d?.USD?.Selling),eur:parseFloat(d?.EUR?.Selling)}) },
        { url:'https://api.exchangerate-api.com/v4/latest/USD', parse:d=>({usd:parseFloat(d?.rates?.TRY),eur:parseFloat(d?.rates?.TRY)/parseFloat(d?.rates?.EUR)}) }
    ];
    for (const src of sources) {
        try {
            const res = await fetch(src.url); if(!res.ok) continue;
            const data = await res.json(); const {usd,eur}=src.parse(data);
            if(usd&&usd>1){
                P.usd=usd; if(eur&&eur>1)P.eur=eur;
                recalcAll(); try{rPar();}catch(e){}
                if(badge)badge.innerHTML=`<i data-lucide="check-circle" class="w-4 h-4 text-[var(--gn)]"></i> Güncel: $${P.usd.toFixed(2)}₺ / €${P.eur.toFixed(2)}₺`;
                lucide.createIcons(); toast('Döviz kurları güncellendi','in'); return;
            }
        } catch(e){}
    }
    if(badge)badge.innerHTML=`<i data-lucide="alert-triangle" class="w-4 h-4"></i> API bağlanamadı — Varsayılan kurlar aktif`;
    lucide.createIcons();
}
...
function recalcAll(){
 IT.forEach(it=>{ if(it._fromCalc) return; calcItem(it); });
 rProd();
...
function updatePrefix(val) { P.prefix = val; const sp=document.getElementById('setPrefix'); if(sp)sp.value=val; const pi=document.getElementById('prefInput'); if(pi)pi.value=val; rPL(); rProd(); toast('Ön ek güncellendi', 'in'); saveData(); }
...
function chGonderFiyatListesi(){
  ...
  // Birim fiyatlar (qt=1 için)
  const us1 = r.mc?.satis || 0;
  const uc1 = r.mc?.toplam || 0;

  const item = {
    ref: RN++, ty: 'manual_pad', dn: dn, cl: 16, qt: qt, df: _chDf,
    parts: [{
      nm: desc, ov:{}, matOv:{}, bw:0, bx:0, by:0,
      calc:{
        S: r.S1||r.We||0, T: r.T1||r.He||0, I:0, N:0,
        fab: (r.altK||0)+(r.ustK||0), fil: r.dolgu||0,
        sew: r.dikis||r.dikis_m||0, drw: r.bogm||0,
        vlc: r.cirt||0, ag: r.agAd||0, pl: r.pulAd||0,
        mc: r.mc?.malzeme||0, lc: r.mc?.iscilik||0, lh: 0
      }
    }],
    mh: '',
    // calc direkt set ediyoruz, recalcAll ezmesin diye _fromCalc flag
    calc: {
      tMC: r.mc?.malzeme||0, tLC: r.mc?.iscilik||0, tLH: 0,
      uc: uc1, us: us1,
      tMA: (r.mc?.malzeme||0)*qt, tLA: (r.mc?.iscilik||0)*qt,
      tS: us1*qt
    },
    _fromCalc: true,
  };
  ...
  // recalcAll _fromCalc itemleri ezip ₺0 yapmasın diye calcItem'ı override ediyoruz
  const origCalc = calcItem;
  IT.forEach(it=>{ if(it._fromCalc) return; origCalc(it); });

  rProd();
  const ap2=document.querySelector('.pane.on');
  if(ap2){if(ap2.id==='p-pl')rPL(); if(ap2.id==='p-ms')rMas();}
  const tI=IT.length, tJ=IT.reduce((s,i)=>s+i.qt,0);
  document.getElementById('sI').textContent=tI;
  document.getElementById('sJ').textContent=tJ;

  toast(`"${desc}" listeye eklendi ✓`,'ok');
...
// ====== EKSİK FONKSİYONLAR (FIXES) ======

function toggleSidebar(){
    const sb=document.getElementById('sidebar');
    const ic=document.getElementById('collapseIcon');
    sb.classList.toggle('collapsed');
    if(ic) ic.setAttribute('data-lucide', sb.classList.contains('collapsed') ? 'chevron-right' : 'chevron-left');
    lucide.createIcons();
}
