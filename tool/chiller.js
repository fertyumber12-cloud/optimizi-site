...
export default function handler(req, res) {
    ...
    try {
        ...
        // ── DOĞRULAMA ──
        if (cop !== undefined && (cop <= 0 || cop > 10)) {
            return res.status(400).json({ hata: `COP 0-10 arasında olmalı: ${cop}` });
        }
        if (hours > 8760) {
            return res.status(400).json({ hata: `Çalışma saati 8760'ı aşamaz: ${hours}` });
        }
