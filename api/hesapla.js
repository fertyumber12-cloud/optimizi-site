...
const MATS={rockwool:{a:0.035,b:0.00017,c:0,maxTemp:650},glasswool:{a:0.025,b:0.00015,c:0,maxTemp:250},ceramic:{a:0.025,b:0.00012,c:1.2e-7,maxTemp:1200},aerogel:{a:0.019,b:0.00004,c:5e-8,maxTemp:650},rubber:{a:0.034,b:0.000075,c:0,maxTemp:105},bare:{a:25,b:0,c:0,maxTemp:9999}};
...
function validateBackend(params) {
    ...
    if (eff !== undefined && (eff <= 0 || eff > 1)) {
        return { block: true, message: `Verim 0-1 arasında olmalı: ${eff}` };
    }
    if (hours !== undefined && (hours <= 0 || hours > 8760)) {
        return { block: true, message: `Çalışma saati 1-8760 arası olmalı: ${hours}` };
    }
    if (qty !== undefined && qty <= 0) {
        return { block: true, message: `Miktar 0'dan büyük olmalı` };
    }
...
export default function handler(req, res) {
    ...
    try {
        ...
        return res.status(200).json({
            ...
            co2SavedTotal, realBareTemp, realInsTemp, Q_bare_unit, Q_unit, roiCalc,
            warnings
        });
