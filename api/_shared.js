// ============================================================
// OPTİMİZİ — SHARED CONSTANTS (ESM — Vercel API)
// Tüm API route'ları bu dosyayı import eder.
// Sabitleri DEĞİŞTİRİRSEN → shared/constants.js'i de güncelle!
// ============================================================
...
function getDN(od_mm) { return Object.keys(PIPES).find(k => PIPES[k] == od_mm) || null; }

function getSurfaceCoeff(std, vWind, Ts, Ta, D_out, isFlat) {
    const dT = Math.abs(Ts - Ta);
    const sigma = PHYSICS.STEFAN_BOLTZMANN;
    const eps = PHYSICS.EMISSIVITY;
...
export { PIPES, SEPARATORS, MATS, VALVE_FACTORS, FUEL_PARAMS, PHYSICS, getK, getOD, getDN, getSurfaceCoeff };
